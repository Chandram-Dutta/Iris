...
class TestGame: Game {
    ...
    func draw(_ g: Graphics, debug: DebugInfo) {
        ...
        g.drawText(
            "Use WASD or Arrow Keys to move", x: 60, y: 90, font: debugFont,
            color: Color(r: 0.7, g: 0.7, b: 0.7))

        g.drawText("FPS: \(debug.fps)", x: 650, y: 60, font: debugFont, color: .green)
        g.drawText(
            "Frame: \(debug.frameNumber)", x: 620, y: 80, font: debugFont,
            color: Color(r: 0.5, g: 0.5, b: 0.5))

        g.drawText(
            "Position: (\(Int(spriteX)), \(Int(spriteY)))", x: 60, y: 530, font: debugFont,
